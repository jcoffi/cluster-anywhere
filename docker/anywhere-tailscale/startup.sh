...
if [ -c /dev/net/tun ] || [ -c /dev/tun ]; then
    ...
    sudo tailscale up --operator=ray --auth-key=$TS_AUTHKEY --accept-risk=all --accept-routes --ssh --stateful-filtering=false
else
    ...
    sudo tailscale up --operator=ray --auth-key=$TS_AUTHKEY --accept-dns=true --accept-risk=all --accept-routes --ssh --stateful-filtering=false
